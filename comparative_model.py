...
class self_attention_model_ner_part():
    ...
    def model(self):
        ...
        word_embedding = TrigPosEmbedding (mode=TrigPosEmbedding.MODE_ADD)(word_embedding)
        # char_embedding = Position_Embedding()(char_embedding)
        ...
        # cnn = MaskedConv1D(filters=self.hidden_size,kernel_size=3,padding='same',activation='relu')(embedding)
        # lstm = Bidirectional(MaskedLSTM(units=self.hidden_size // 2, return_sequences=True), name='lstm_layer0')(cnn)
        # self_att = MultiHead(self_attention(units=self.hidden_size),layer_num=3)(cnn)

        inputs = embedding

        for i in range(self.multi_layers):
            residual = inputs
            self_att = MultiHeadAttention(head_num=8)(embedding)
            self_att = BatchNormalization()(self_att)
            self_att = Add()([residual,self_att])
            self_att = Dropout(self.nn_dropout_prob)(self_att)

            residual = self_att
            feed_forward_layer = FeedForward(self.hidden_size)(residual)
            feed_forward_layer = BatchNormalization()(feed_forward_layer)
            inputs =  Add()([residual,feed_forward_layer])
            inputs = Dropout(self.nn_dropout_prob)(inputs)
        ...
        #     for i in range(self.multi_layers-1):
        #         i = i+1
        #         # lstm = Bidirectional(MaskedLSTM(units=self.hidden_size // 2, return_sequences=True), name='lstm_layer{}'.format(i))(self_att)
        #         cnn = MaskedConv1D(filters=self.hidden_size, kernel_size=3, padding='same', activation='relu')(self_att)
        #         # self_att = self_attention(units=self.hidden_size)(lstm)
        #         self_att = MultiHeadAttention(head_num=4, name='multi_head_att{}'.format(i))(cnn)

        bio_pred = Dense(self.num_classes, activation='softmax')(inputs)
